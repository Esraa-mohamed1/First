...
import { enrollInCourse, subscribeToCourse } from '@/services/student-courses';
...
export const PaymentMethodModal = ({
  ...
  const isFreeCourse = Number(coursePrice) === 0 || coursePrice === '0' || coursePrice === 'free' || !coursePrice;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isFreeCourse && !screenshot) { showAlert.error('مطلوب الإيصال', 'يرجى رفع صورة الإيصال'); return; }
    
    setLoading(true);
    try {
      if (isFreeCourse) {
        await subscribeToCourse(courseId, 0);
      } else {
        if (!selectedMethod) return;
      ...
      await showAlert.success(
        isFreeCourse ? 'تم الاشتراك بنجاح 🎉' : 'تم إرسال طلب التسجيل ✅',
        isFreeCourse ? 'تم تفعيل الدورة المجانية ويمكنك البدء في التعلم الآن!' : 'سيتم مراجعة الإيصال وتفعيل الدورة قريباً.'
      );
      handleClose();
    } catch (error: any) {
      showAlert.error('فشل الاشتراك', error?.message || 'حدث خطأ، يرجى المحاولة لاحقاً');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black text-gray-900">
                {isFreeCourse ? 'الاشتراك في الدورة المجانية' : step === 1 ? 'اختر وسيلة الدفع' : 'رفع إيصال التحويل'}
              </h2>
              {!isFreeCourse && <p className="text-[10px] text-gray-400 font-bold mt-0.5">الخطوة {step} من 2</p>}
            </div>
          </div>
          <div className="flex items-center gap-3">
            {!isFreeCourse && (
        ...
        {/* ── Free Course View ── */}
        {isFreeCourse ? (
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 text-right" dir="rtl">
            <div className="p-8 flex flex-col items-center text-center space-y-4 my-auto">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900">الدورة مجانية بالكامل 🎉</h3>
                <p className="text-xs text-slate-500 font-bold mt-1.5 max-w-sm leading-relaxed">يمكنك الاشتراك المباشر فوراً والوصول لجميع محتويات الدورة بدون الحاجة لإرفاق إيصال دفع أو اختيار حساب تحصيل.</p>
              </div>
              <div className="bg-emerald-50/80 border border-emerald-200/80 text-emerald-800 text-xs font-black px-5 py-3 rounded-2xl w-full">
                رسوم الاشتراك: مجاناً (0 {courseCurrency || 'SAR'})
              </div>
            </div>
            ...
            <div className="p-5 border-t border-gray-100 shrink-0">
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-black rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 cursor-pointer"
              >
                {loading ? <><Loader2 className="animate-spin" size={20} /> جاري تفعيل الاشتراك...</> : 'اشترك الآن مجاناً'}
              </button>
            </div>
          </form>
        ) : (
