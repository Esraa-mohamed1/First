...
interface AddCoachModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (coach: User) => void;
  onCoachAdded?: (coach: User) => void;
}
...
export default function AddCoachModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const createdCoach = await createUser(payload);
      toast.success('تمت إضافة المدرب بنجاح');
      if (onSuccess) onSuccess(createdCoach);
      if (onCoachAdded) onCoachAdded(createdCoach);
      onClose();
      // Reset form
      setFormData({
        name: '',
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-gray-900">إضافة مدرب جديد</h2>
              <p className="text-sm font-bold text-gray-400 mt-1">قم بإدخال بيانات المدرب لإنشاء حسابه</p>
            </div>
          </div>
          <button
            onClick={onClose}
