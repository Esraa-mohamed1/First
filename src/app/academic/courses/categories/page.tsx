...
export default function CategoriesPage() {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6" dir="rtl">
      ...
      {/* Add Category Input */}
      {isAdding && (
        <div className="bg-white rounded-2xl border border-blue-100 p-6 flex flex-col md:flex-row items-center gap-4 shadow-sm animate-in fade-in slide-in-from-top-2">
           <div className="flex-1 w-full">
              <input 
                type="text" 
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                placeholder="ادخل اسم الفئة الجديدة"
                className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-600 font-bold transition-all"
                autoFocus
              />
           </div>
           <div className="flex gap-2 w-full md:w-auto">
              <button 
                ...
                className="flex-1 md:flex-none bg-blue-600 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
              >
              ...
              <button 
                ...
                className="flex-1 md:flex-none bg-gray-100 text-gray-500 px-6 py-3 rounded-xl font-bold text-sm hover:bg-gray-200 transition-all flex items-center justify-center gap-2"
              >
                <X size={16} />
                إلغاء
              </button>
                  ...
                  {editingId === category.id ? (
                     <div className="flex items-center gap-2">
                        <input 
                          ...
                          className="flex-1 bg-gray-50 border border-blue-200 rounded-lg px-2 py-1 text-sm outline-none font-bold"
                          autoFocus
                        />
