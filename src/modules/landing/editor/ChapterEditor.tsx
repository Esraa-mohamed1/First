...
export default function ChapterEditor() {
  ...
  return (
    <div className="space-y-6 text-right font-sans" dir="rtl">
      <div>
        <h3 className="text-sm font-black text-slate-800 border-r-4 border-blue-600 pr-2">تعديل محتوى ومنهج الدورة</h3>
        <p className="text-[10px] text-slate-400 font-bold mt-1">خصص مظهر جدول الدروس والمحاضرات للطلاب</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-100">
          <div>
            <label className="text-xs font-black text-slate-900 block">عرض تفاصيل قائمة الدروس</label>
            <span className="text-[10px] text-slate-400 font-bold">إتاحة للطلاب تصفح أسماء الدروس والمدد الزمنية</span>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              className="sr-only peer"
              checked={data.showLessons}
              onChange={(e) => handleChange('showLessons', e.target.checked)}
            />
